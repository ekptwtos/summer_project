...
#        print("--- %s seconds ---" % (time.time() - start_time))
    ...
    def declutter_scene(self,index, tar_att):
...
#            self.aro = None
    ...
    def grasp_generator(self, initial_poses, attempt):
...
#        threading.Timer(0.5, self.scene_generator).start()
    ...
    def sort_list(self, the_list):
        ...
        x1 = obj_pose[obj_id.index('target')].pose.position.x
        y1 = obj_pose[obj_id.index('target')].pose.position.y
#        z1 = obj_pose[obj_id.index('target')].pose.position.y
#        tar_point =np.array((x1, y1, z1))

        distances = []
        distances2 = []
        for i in the_list:
            x2 = obj_pose[i].pose.position.x
            y2 = obj_pose[i].pose.position.y
#            z2 = obj_pose[i].pose.position.z
#            obj_point = np.array((x2, y2, z2))
            distances.append(math.hypot(x2-x1, y2-y1))
#            distances2.append(np.linalg.norm(tar_point-obj_point))

        distances, the_list = zip(*sorted(zip(distances, the_list)))

        ### RETURN THE RESULTS BACK TO LIST FORM ###
        distances, the_list = (list(t) for t in zip(*sorted(zip(distances, the_list))))
